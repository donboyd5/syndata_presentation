...
edir <- "C:/Users/donbo/Dropbox/SLGF/taxdata_synpuf/" # enhanced files are stored here
epuf_fn <- "puf_rottenpuf.csv"
esyn_fn <- "puf_synpuf.csv"

epuf_path <- paste0(edir, epuf_fn)
esyn_path <- paste0(edir, esyn_fn)
...
esr_samp_path <- paste0(edir, "estack_rotten_samp.csv")
esr_path <- paste0(edir, "estack_rotten.csv")
...
# stackpath <- "C:/Users/donbo/Dropbox/SLGF/taxdata_synpuf/estack_rotten_samp.csv"
stackpath <- "C:/Users/donbo/Dropbox/SLGF/taxdata_synpuf/estack_rotten.csv"
...
(tc_samp_path <- paste0(taxout, "estack_rotten_samp-18-#-#-#.csv"))
esr_samp_path

(tc_path <- paste0(taxout, "estack_rotten-18-#-#-#.csv"))
esr_path

# use either the sample or full output
# f1 <- esr_samp_path; f2 <- tc_samp_path # sample output
f1 <- esr_path; f2 <- tc_path # full output
check <- read_csv(f1) %>% 
  select(ftype, RECID, RECID_original, s006) %>%
  left_join(read_csv(f2) %>%
              select(RECID, taxbc))

check %>% ht
check %>% 
  group_by(ftype) %>%
  summarise(taxbc=sum(s006 / 100 * taxbc) / 1e9) %>%
  spread(ftype, taxbc) %>%
